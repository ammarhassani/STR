...
def test_panel_controller():
    ...
    try:
        ...
        # a fresh heartbeat is present -> restore refuses (never restore over a live host)
        check("restore refuses while host live", pc.restore_backup(pc.list_backups()[0])[0] is False)
        okr, msgr = pc.restore_backup(pc.list_backups()[0], force=True)
        check("restore ok with force", okr, msgr)
        check("restore refuses unknown", pc.restore_backup("nope.db", force=True)[0] is False)
...
def test_operator_docs_cover_the_real_procedures():
    """The operator guides must actually answer the questions they exist for.

    These replaced HOST_RUNBOOK.md, which described the .vbs launchers that no
    longer exist. A guide that documents a removed mechanism is worse than no
    guide: it sends an operator looking for a file that is not there.
    """
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    for rel in ("deploy/start_host.bat", "deploy/start_panel.bat",
                "docs/SETUP.md", "docs/OPERATIONS.md", "docs/TEST_DAY.md"):
        check(f"{rel} exists", os.path.exists(os.path.join(root, rel)), rel)

    ops = open(os.path.join(root, "docs/OPERATIONS.md"), encoding="utf-8").read().lower()
    for term in ("host", "back up", "restore", "shared folder", "control panel"):
        check(f"OPERATIONS.md covers '{term}'", term in ops)
    check("OPERATIONS.md explains taking over when the host dies",
          "make this pc the host" in ops)

    setup = open(os.path.join(root, "docs/SETUP.md"), encoding="utf-8").read().lower()
...
if __name__ == "__main__":
    ...
    test_operator_docs_cover_the_real_procedures()
