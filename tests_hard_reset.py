...
def test_runbook_documents_reset_and_golden_path():
    # #21 + #22 doc-sync: the runbook must carry the golden path and the reset.
    rb = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           'docs/HOST_RUNBOOK.md'), encoding='utf-8').read()
    check("runbook has a golden-path 'do this first' section", 'Do This First' in rb, )
    check("runbook documents the hard reset", 'Hard Reset' in rb and 'reset_to_production.py' in rb)
    check("golden path covers host, client, first login, updates",
          all(k in rb for k in ['Host', 'Client', 'admin123', 'git pull']))


if __name__ == "__main__":
    test_hard_reset()
    test_runbook_documents_reset_and_golden_path()
