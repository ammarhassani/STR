"""#6 — log export actually writes a file. Run: python3.14 tests_log_export.py"""
...
def test_export_logs_writes_file():
    ...
    check("filename is a timestamped csv", str(path).endswith('.csv') and 'fiu_logs_' in str(path), path)

    # read it back (utf-8-sig, as written)
